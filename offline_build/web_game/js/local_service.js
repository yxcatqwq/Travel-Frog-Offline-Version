...
/* ---- 63_activities.js ---- */
    ...
    animpicture.add = function (work, params, effects) { params=util.isObject(params)?params:{}; var entry=animpicture.entry(work,params.index!==undefined?params.index:params.anim_index); if(!entry)return {ok:false,code:LF.ERR.ILLEGAL_PARAM,reason:'anim-index'}; var ids=Array.isArray(params.pic_ids)?params.pic_ids:(params.pic_id!==undefined?[params.pic_id]:[]), list=animpicture.photoList(work), moved=[]; for(var i=0;i<ids.length;i++){var at=animpicture.photoIndex(list,ids[i]);if(at>=0){moved.push(list.splice(at,1)[0]);}} if(!moved.length)return {ok:false,code:LF.ERR.ILLEGAL_OP,reason:'picture-not-found'}; entry.exp_pic=Array.isArray(entry.exp_pic)?entry.exp_pic:[]; Array.prototype.push.apply(entry.exp_pic,moved); rules.effect(effects,'activities'); rules.effect(effects,'mail'); return {ok:true,code:LF.ERR.OK,exp_pic:util.clone(entry.exp_pic)}; };
    animpicture.remove = function (work, params, effects) { params=util.isObject(params)?params:{}; var entry=animpicture.entry(work,params.index!==undefined?params.index:params.anim_index); if(!entry)return {ok:false,code:LF.ERR.ILLEGAL_PARAM,reason:'anim-index'}; var ids=Array.isArray(params.pic_ids)?params.pic_ids:(params.pic_id!==undefined?[params.pic_id]:[]), list=animpicture.photoList(work), keep=[], removed=[]; entry.exp_pic=Array.isArray(entry.exp_pic)?entry.exp_pic:[]; for(var i=0;i<entry.exp_pic.length;i++){if(ids.length===0||ids.some(function(id){return String(id)===String(entry.exp_pic[i]&&entry.exp_pic[i].id);}))removed.push(entry.exp_pic[i]);else keep.push(entry.exp_pic[i]);} if(!removed.length)return {ok:false,code:LF.ERR.ILLEGAL_OP,reason:'picture-not-found'}; entry.exp_pic=keep; if(!params.is_delete)Array.prototype.push.apply(list,removed); rules.effect(effects,'activities'); rules.effect(effects,'mail'); return {ok:true,code:LF.ERR.OK,exp_pic:util.clone(entry.exp_pic)}; };
